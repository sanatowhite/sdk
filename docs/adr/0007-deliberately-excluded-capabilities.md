...
- ~~**No authentication.** Out of scope entirely — this is a network-layer template (`:core-net`), not an identity-and-session template. Adding auth is left to the fork, since auth architecture (OAuth vs. custom, token refresh strategy, session storage) varies too much to templatize usefully.~~ **Superseded by [ADR 0012](0012-firebase-auth-and-websocket.md)**: the variance this bullet worried about is mostly a self-built-backend problem. A fully-hosted vendor (Firebase Auth) collapses it the same way a hosted vendor already collapsed the telemetry-backend variance in ADR 0006, so `:core-auth`/`:auth-firebase`/`:auth-net-hilt`/`:feature-auth` now ship as real published modules.
