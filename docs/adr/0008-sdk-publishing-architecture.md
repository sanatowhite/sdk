...
- Coordinate form: `com.github.sanatowhite.sdk:<module>:<version>` for **every** publishable module, including `:updatechecker`, which now shares the same publish set, same tag, same version as everything else instead of being mirrored out separately. **ADR 0001 is superseded**: the subtree-mirror mechanism is gone; old tags `v1.0.0`/`v1.0.1`/`v1.0.2` were initially kept on the reasoning that JitPack caches builds permanently (deleting them only turns "resolves" into "404" for whoever's still pinned to them), but were deleted anyway shortly after the rename/re-publish, as an explicit decision to not carry the dead coordinate indefinitely — documented as deprecated with no forward compatibility bridge to the new coordinate either way.
