...
应用内 Debug Drawer:可拖拽把手 + 抽屉,内容是本地 feature flag 覆写 + 崩溃/ANR/OOM 一键触发 + 启动/卡顿/网络日志(读 `:core-telemetry` 的 `RingLogBuffer`)+ LogKit 面板(读 `:logkit` 的静态入口,验证并发顺序一致性/5MB 滚动淘汰/崩溃-ANR 持久性/可解密性四个属性)。只在 `debugImplementation` 下参与构建。
...
- LogKit 面板(`DebugDrawerContent.kt` 内的 `LogKitPanel`,不对外单独导出)—— 并发压测按钮(4×1000/8×5000/16×20000)、5MB 预算进度条 + 文件列表 + `Fill 6MiB`/`Purge all`、崩溃/ANR 触发(复用 `CrashTriggers`)、`Export & share`(走和 `FeedbackViewModel` 一样的 `${applicationId}.fileprovider` 路径)。
...
- **不要**指望 LogKit 面板能自己验证"顺序一致性"——记录已加密,进程内没有私钥,面板只报告写入/丢弃/耗时,连续性必须离线跑 `logkit-decrypt --verify-seq` 验证。
