...
@Composable
internal fun DebugDrawerContent(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        Row(
            ...
            IconButton(onClick = onClose) {
                Icon(Icons.Outlined.Close, contentDescription = "Close debug drawer")
            }
        }
        ...
        // `:logkit` 不是这个(已发布)模块的依赖——它是本仓库私有、未发布的模块
        // (见 CLAUDE.md ":logkit 五条铁律" #1),`:debug-tools` 一旦依赖它就会被
        // `verifyModuleGraph` 的发布正确性检查(publishable 模块不能依赖
        // non-publishable 模块)拦下来。[extraContent] 是消费方自带内容的挂载点,
        // `:app` 的 debug 门面用它挂 LogKit 调试面板,`:debug-tools` 本身对
        // `:logkit` 零感知。
        HorizontalDivider(modifier = Modifier.padding(vertical = 8.dp))
        extraContent()
    }
